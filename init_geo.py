...
def main(source_path, model_path, ckpt_path, device, batch_size, image_size, schedule, lr, niter, 
    ...
    # extract depth maps
    depthmaps_2d = [d.detach().cpu().numpy() for d in scene.get_depthmaps(raw=False)]
    
    # save the depth maps
    depth_out = Path(sparse_0_path) / "depth_maps"
    depth_out.mkdir(exist_ok=True)
    
    # assume image_files is your list of filenames in the same order as scene.get_depthmaps()
    for name, dmap in zip(image_files, depthmaps_2d):
        # save as a .npy
        np.save(depth_out / f"{Path(name).stem}_depth.npy", dmap)
    
        # optionally save a 16-bit PNG for visualization
